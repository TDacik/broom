(* Testcase building a simple formula with 
Slseg(%l1, %l2, lambda-1:1, [%l3])  * %l3 -(8)->Undef 
...
let () =
	...
	in
	let result_folded = {sigma = [	Slseg(Var 1, Var 2, lambda, [Var 3]); 
									Hpointsto(Var 3, ptr_size, Undef)];
									 pi = []}
	in
