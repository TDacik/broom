(* Testcase building a formula with nodes containg the linking field and a data element
*)
...
let () =
	...
	let form = {
		sigma = [Hpointsto(BinOp (Pplus, Var 1, ptr_size), ptr_size, Const(Int 1L));
				 Hpointsto(BinOp (Pplus, Var 2, ptr_size), ptr_size, Const(Int 1L));
