...
let const_to_solver ctx c =
  ...
  | Exp.String _ -> (* handle by expr_to_solver *)
...
let rec expr_to_solver ctx func expr =
  ...
  | Exp.Const (String _) -> expr_to_solver ctx func (Exp.Undef)
    (* TODO: not UNDEF, more precise *)
...
let try_simplify_SL_expr_to_int solver form expr =
   ...
   if (Solver.check solver.solv query)==SATISFIABLE
   then 
   	match Solver.get_model solver.solv with
	| None -> None
	| Some model ->
		(match (Model.get_const_interp_e model newvar) with
		| None -> None
		| Some value ->  
