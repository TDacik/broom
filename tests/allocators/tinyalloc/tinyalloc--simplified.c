...
// small test program for ta_alloc/free
...
int main(void) {
	...
	// get some raw memory
	void *base=malloc(10000);
	if(base==NULL) return 11;
	void *limit=(char*)base+10000;
	// initialization of ta allocator
        if(ta_init(base, limit, 10 /*heap_blocks*/, 5 /*split_thresh*/, 8 /*alignment*/))
	       return 12;	

	char *p1 = ta_alloc(100);
	if(p1==NULL) return 1;
	char *p2 = ta_alloc(1000);
	if(p2==NULL) return 2;
