...
//     };
// beg rewrite
    PRUint8 * pmasks = alloca(sizeof(PRUint8) * 33);
    pmasks[0] = 0;
    pmasks[1] = 0;
    pmasks[2] = 1;
    pmasks[3] = 3;
...
// TODO: simple test
int main() {
    PLArenaPool * p_pool = alloca(sizeof(PLArenaPool));
//     PLArenaPool pool;
    const unsigned size = 4096*32;
    const unsigned align = 16;

    PL_InitArenaPool(p_pool, "name", size, align);

    void * p1 = PL_ArenaAllocate(p_pool, 60);
    printf("%p\n", p1);

    void * p2 = PL_ArenaAllocate(p_pool, 70);
    printf("%p\n", p2);

#if 1
    void * p3 = PL_ArenaAllocate(p_pool, size-10);
    printf("%p\n", p3);
#endif

    PL_FinishArenaPool(p_pool);
}
#endif
