...
{
    CC_List *list = calloc(1, sizeof(CC_List));
...
//     conf.mem_free   = list->mem_free;
    ...
    CC_List *sub;
    enum cc_stat status = cc_list_new(&sub);
...
//     conf.mem_free   = list->mem_free;
    ...
    CC_List *copy;
    enum cc_stat status = cc_list_new(&copy);
...
enum cc_stat cc_list_zip_iter_add(CC_ListZipIter *iter, void *e1, void *e2)
{
    Node *new_node1 = calloc(1, sizeof(Node));

    if (!new_node1)
        return CC_ERR_ALLOC;

    Node *new_node2 = calloc(1, sizeof(Node));
