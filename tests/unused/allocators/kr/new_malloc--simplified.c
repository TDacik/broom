...
// abstract base address for heap (we expect correct initialization)
...
void *new_malloc(unsigned required_size) {
  ...
  if (free_list == NULL) {
    ...
    //## allocate HEAP using system malloc() => LEAK if not freed in main
    memory = malloc(MEMSIZE); if (memory==NULL) abort();
...
#ifdef TEST
// small test program for malloc/free

#include <stdio.h>
#include <stdlib.h>

int main(void) {
	char *p1 = new_malloc(100);
	if(p1==NULL) return 1;
	char *p2 = new_malloc(1000);
	if(p2==NULL) return 2;
	...
	*p2 = 'a';
	...
	new_free(p2);	// no LIFO order

#ifdef DBLFREE
	// double free
	new_free(p1);
#endif
	free(memory);  	//## no leak of allocated raw memory
}
